...
//                    strcpy(_server_info.host, server_env->host);
                ...
                strcpy(_server_info.host, inet_ntoa(((struct sockaddr_in*)&remote->direct_addr.addr)->sin_addr));

                if (verbose) {
                    LOGI("server_info host %s", _server_info.host);
                }
                _server_info.port = ((struct sockaddr_in*)&remote->direct_addr.addr)->sin_port;;
...
//
//        int i = 0;
//        while ((remote->send_ctx->remote->server->buf->array)[i] != '\0' || (i == 2)) {
//            if (i == 2) {
//                str[i] = 'O';
//            } else {
//                str[i] = (remote->send_ctx->remote->server->buf->array)[i];
//            }
//            if (1 == 99) {
//                break;
//            }
