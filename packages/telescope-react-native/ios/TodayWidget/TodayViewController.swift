//
//  TodayViewController.swift
//  TodayWidget
//
//  Created by LEI on 4/12/16.
//  Copyright © 2016 TouchingApp. All rights reserved.
//

import UIKit
import NotificationCenter
import ShadowBase
import SnapKit
import SwiftColor
import ShadowLibrary
import MMWormhole
import CocoaAsyncSocket

private let kCurrentGroupCellIndentifier = "kCurrentGroupIndentifier"

class TodayViewController: UIViewController, NCWidgetProviding, UITableViewDataSource, UITableViewDelegate, GCDAsyncSocketDelegate {
        
    let wormhole = Manager.sharedManager.wormhole
    
    var timer: Timer?
    
    var rowCount: Int {
        return 2
    }
    
    var status: Bool = false
 
    var socket: GCDAsyncSocket!

    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        super.init(nibName: nibNameOrNil, bundle: nibBundleOrNil)
        socket = GCDAsyncSocket(delegate: self, delegateQueue: DispatchQueue.global(qos: DispatchQoS.QoSClass.background))
    }

    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        let port = Shadow.sharedUserDefaults().integer(forKey: "tunnelStatusPort")
        status = port > 0
        tableView.register(CurrentGroupCell.self, forCellReuseIdentifier: kCurrentGroupCellIndentifier)
        view.addSubview(tableView)
        updateLayout()
    }
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        startTimer()
        tableView.reloadData()
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        stopTimer()
    }

    @objc func tryConnectStatusSocket() {
        let port = Shadow.sharedUserDefaults().integer(forKey: "tunnelStatusPort")
        guard port > 0 else {
            updateStatus(false)
            return
        }
        do {
            socket.delegate = self
            try socket.connect(toHost: "127.0.0.1", onPort: UInt16(port))
        }catch {
            updateStatus(false)
        }
    }

    func startTimer() {
        timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(TodayViewController.tryConnectStatusSocket), userInfo: nil, repeats: true)
        timer?.fire()
    }

    func stopTimer() {
        socket.disconnect()
        timer?.invalidate()
        timer = nil
    }

    // MARK: - Socket

    func socket(_ sock: GCDAsyncSocket, didConnectToHost host: String, port: UInt16) {
        updateStatus(true)
        sock.delegate = nil
        sock.disconnect()
    }

    func socketDidDisconnect(_ sock: GCDAsyncSocket, withError err: Error?) {
        updateStatus(false)
    }

    func updateStatus(_ current: Bool) {
        if status != current {
            status = current
            DispatchQueue.main.async(execute: { 
                self.tableView.reloadRows(at: [IndexPath(row: 0, section: 0)], with: .none)
            })
        }
    }
    
    func switchVPN() {
        if status {
            wormhole.passMessageObject("" as NSCoding?, identifier: "stopTunnel")
        }else {
            Manager.sharedManager.loadProviderManager { (manager) -> Void in
                if let manager = manager {
                    if manager.isEnabled {
                        URLSession.shared.dataTask(with: URL(string: "https://on-demand.connect.aerowings.vip/start/")!).resume()
                    } else {
                        Manager.sharedManager.switchVPNFromTodayWidget(self.extensionContext!)
                    }
                }
            }
        }
    }
    
    func widgetMarginInsets(forProposedMarginInsets defaultMarginInsets: UIEdgeInsets) -> UIEdgeInsets {
        var inset = defaultMarginInsets
        inset.bottom = 0
        return inset
    }

    func widgetPerformUpdate(_ completionHandler: (@escaping (NCUpdateResult) -> Void)) {
        // Perform any setup necessary in order to update the view.

        // If an error is encountered, use NCUpdateResult.Failed
        // If there's no update required, use NCUpdateResult.NoData
        // If there's an update, use NCUpdateResult.NewData
        
        completionHandler(NCUpdateResult.newData)
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return rowCount
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell: UITableViewCell!
        if indexPath.row == 0 {
            cell = tableView.dequeueReusableCell(withIdentifier: kCurrentGroupCellIndentifier, for: indexPath)
            let selectNodeRemark = Shadow.sharedUserDefaults().object(forKey: kSelectNode) as? String
            (cell as? CurrentGroupCell)?.config(selectNodeRemark ?? "Default".localized(), status: status, switchVPN: switchVPN)
        } else if indexPath.row == 1 {
            cell = tableView.dequeueReusableCell(withIdentifier: kCurrentGroupCellIndentifier, for: indexPath)
            (cell as? CurrentGroupCell)?.configOnlyName("Choose Node".localized())
        }
        cell.preservesSuperviewLayoutMargins = false
        cell.layoutMargins = UIEdgeInsets.zero
        cell.separatorInset = UIEdgeInsets.zero
        cell.selectionStyle = .none
        if indexPath.row == rowCount - 1 {
            cell.separatorInset = UIEdgeInsets(top: 0, left: cell.bounds.size.width, bottom: 0, right: 0)
        }else {
            cell.separatorInset = UIEdgeInsets.zero
        }
        
        return cell
    }
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return 53
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 1 {
            if let url = URL(string: "telescope://choose-node") {
                extensionContext!.open(url, completionHandler: nil)
            }
        }
    }
    
    func updateLayout() {

        tableView.snp.makeConstraints { (make) in
            make.left.equalToSuperview()
            make.top.equalToSuperview()
            make.right.equalToSuperview()
            make.bottom.equalToSuperview().offset(-15)
            make.height.equalTo(CGFloat(44 * rowCount))
        }
    }
    
    lazy var tableView: UITableView = {
        let v = UITableView(frame: CGRect.zero, style: .plain)
        v.tableFooterView = UIView()
        v.tableHeaderView = UIView()
        v.dataSource = self
        v.delegate = self
        return v
    }()
    
    
    
}
