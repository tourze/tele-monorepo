...
open class DNSQuery {
    ...
    init?(payload: Data, offset: Int, base: Int = 0) {
        ...
        let type = scanner.read16()!
        guard let type = DNSType(rawValue: type) else {
            DDLogError("Received DNS packet with unknown type.")
            return nil
        }
        self.type = type

        let tClass = scanner.read16()!
        guard let klass = DNSClass(rawValue: tClass) else {
...
open class DNSResource {
    ...
    init?(payload: Data, offset: Int, base: Int = 0) {
        ...
        let u = scanner.read16()!
        guard let klass = DNSClass(rawValue: u) else {
            DDLogError("Received DNS packet with unknown class.")
            return nil
        }
        self.klass = klass
