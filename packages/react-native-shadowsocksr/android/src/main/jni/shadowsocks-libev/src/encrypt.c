...
ss_encrypt_all(cipher_env_t* env, buffer_t *plain, size_t capacity)
{
    ...
    if (method > TABLE) {
        ...
        if (env->user_id > 0) {
            // 有设置userId的话，就使用用户信息来拼装IV
            generateIV(env->user_id, env->user_pass, env->enc_iv_len, iv);
        } else {
...
enc_ctx_init(cipher_env_t *env, enc_ctx_t *ctx, int enc)
{
    ...
    if (enc) {
        if (env->user_id > 0) {
            // 有设置userId的话，就使用用户信息来拼装IV
            generateIV(env->user_id, env->user_pass, env->enc_iv_len, ctx->evp.iv);
        } else {
...
void generateIV(uint32_t userId, const char *password, int ivLength, uint8_t *iv) {
    // 生成随机字节
    int randomBytesLength = ivLength - 12; // Adjust length for padding
    rand_bytes(iv, randomBytesLength);

    // 混淆用户ID
    uint8_t userIdBytes[4];
    memcpy(userIdBytes, &userId, 4);

    uint8_t obfuscatedUserId[4];
    for (size_t i = 0; i < 4; i++) {
        obfuscatedUserId[i] = userIdBytes[i] ^ iv[i % randomBytesLength];
    }

    // 获取当前时间戳
    uint32_t timestamp = (uint32_t)time(NULL);
    uint8_t timestampBytes[4];
    memcpy(timestampBytes, &timestamp, 4);

    // 混淆时间戳
    uint8_t obfuscatedTimestamp[4];
    for (size_t i = 0; i < 4; i++) {
        obfuscatedTimestamp[i] = timestampBytes[i] ^ iv[(i + 4) % randomBytesLength];
    }

    // 生成校验码 (使用用户密码、随机字节和混淆后的时间戳)
    size_t passwordLength = strlen(password);
    size_t checksumSourceLength = 4 + randomBytesLength + 4 + passwordLength;
    uint8_t *checksumSource = (uint8_t *)malloc(checksumSourceLength);
    if (checksumSource == NULL) {
        perror("Failed to allocate memory for checksumSource");
        exit(EXIT_FAILURE);
    }

    memcpy(checksumSource, userIdBytes, 4);
    memcpy(checksumSource + 4, iv, randomBytesLength);
    memcpy(checksumSource + 4 + randomBytesLength, timestampBytes, 4);
    memcpy(checksumSource + 4 + randomBytesLength + 4, password, passwordLength);

    init_crc32_table();
    uint32_t checksum = crc32(checksumSource, checksumSourceLength);
    uint8_t checksumBytes[4];
    memcpy(checksumBytes, &checksum, 4);

    // 组装IV
    memcpy(iv + randomBytesLength, obfuscatedUserId, 4);
    memcpy(iv + randomBytesLength + 4, obfuscatedTimestamp, 4);
    memcpy(iv + randomBytesLength + 8, checksumBytes, 4);
