...
// 移除应用 BuildConfig 依赖，使用本地 DEBUG 开关
    ...
    fun start(onRestartCallback: (() -> Unit)? = null): GuardedProcess {
        ...
        guardThread = thread(name = "GuardThread-" + cmd.first()) {
            try {
                ...
                while (!isDestroyed) {
                    if (DEBUG) Log.d(TAG, "start process: " + Commandline.toString(cmd))
                ...
                if (DEBUG) Log.d(TAG, "thread interrupt, destroy process: " + Commandline.toString(cmd))
                destroyProcess()
            } catch (e: IOException) {
                ioException = e
            } finally {
