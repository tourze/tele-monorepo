...
- MCP 工具不可用或响应异常：记录错误信息，通知干系人，优先切换到人工评审或同步会议，并登记例外卡；待工具恢复后回放会话。
- 若多轮合作仍无法收敛，可回滚至提问框架阶段，重新整理证据与假设，再启动新会话；必要时升级至负责人决策。
- 对于 Codex 建议导致的错误实验或变更，立即启用原有回滚方案（Kill Switch、回滚脚本），并在复盘中标注建议来源及失效原因。
- 检测到错误传播或上下文混乱时，终止当前会话并清理上下文，重新执行"错误检测机制"自检后再发起协作。
- 如 Codex MCP 在 10 分钟内无响应，可视为超时：撤回请求，将任务拆分、缩小范围或改用离线渠道，再次尝试。
...
- Codex 协作记录：提问框架、调用命令、`conversationId`、关键往返对话摘要。
- 决策日志：采纳与否的结论、验证步骤、命令输出或测试结果。
- 例外卡或风险登记：工具不可用、建议失效、未解决问题的后续处理计划。
- 复盘材料：问题根因、协作成效评估、对技能或流程的改进建议。
