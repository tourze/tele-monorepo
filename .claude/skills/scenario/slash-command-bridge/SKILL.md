...
description: 当需要在非 Claude Code 环境中解析并执行 slash 命令，桥接 `.claude/commands/` 与用户命令目录时，请加载本技能。
...
- 已解析的最终提示文本（含参数替换、前置命令输出），用于驱动后续任务。
- 命令执行日志：定位路径、前置命令执行结果、错误信息（如有）。
- 可选：更新后的命令索引或帮助文档，便于用户查询可用斜杠命令。
