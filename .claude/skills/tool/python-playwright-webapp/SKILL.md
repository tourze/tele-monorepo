...
- 测试脚本引入不稳定操作导致频繁失败时，回退到上一个稳定版本，增加显式等待或断言再提交。
- 被测服务异常或测试数据污染，应恢复到初始化数据快照或重新拉起干净环境后再运行测试。
- Playwright 浏览器版本不兼容，执行 `playwright install --with-deps` 同步内核；必要时锁定版本并记录原因。
- E2E 测试阻塞发布时，提供降级或跳过策略（如标记 `xfail`、拆分冒烟集），并登记后续补测计划。
