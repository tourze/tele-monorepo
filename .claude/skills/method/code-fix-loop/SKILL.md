...
description: 指导在多语言项目中执行“定位 → 分析 → 修复 → 回归 → 复盘”的缺陷治理闭环。
---
...
### 并行推进要求
- 按照 `method/parallel-task-coordination` 规划批次与任务队列，明确并行粒度、依赖关系与验收口径。
- 在命中多个问题时，立即创建子代理或多任务执行器，并在各轮总结中同步进度、风险与剩余行动。
- 并行任务完成后需合并验证结果，确保质量门覆盖全部改动；若存在冲突，优先保障业务正确并记录处理策略。
...
**示例**:
```bash
# 验证PHP trait属性在继承链中的可见性
php -r 'trait T { private $x; }
abstract class A { use T; }
class B extends A {}
$r = new ReflectionClass(B::class);
echo count($r->getProperties());'
# 输出:0 → 确认子类无法访问父类trait属性
```
...
**实战经验**：
- **wechat-work-intercept-rule-bundle案例**：复杂度35→3，提取8个子方法
- **重构风险控制**：预先分析类型签名，避免引入argument.type错误
- **验证节奏**：每个子方法提取后立即验证，不累积风险
6. **回归验证**
   - **建立测试基线**：修复前运行测试并记录状态（通过/失败/跳过），作为对比基准。
   - **对比验证**：修复后再次运行，确认仅修复目标问题且无新增失败或回归。
   - **集成验证优先**：当质量门或 PHPStan 要求集成测试覆盖时，必须执行指定集成测试套件，禁止用单元测试替代，并记录命令与结果。
   - 先红后绿：重现缺陷 → 应用修复 → 再运行验证，确保问题已解决且无回归。
   - 每个迭代闭环都需执行一次完整收尾测试（含目标模块回归与关键冒烟），并写入取证记录。
   - 对跨模块风险执行额外验证（性能、安全、可观测性等）。
7. **复盘沉淀**
   - 汇总根因、修复要点、残留风险、后续改进。
   - 如触发例外卡或临时豁免，调用 `scenario-exception-card` 完成登记。
