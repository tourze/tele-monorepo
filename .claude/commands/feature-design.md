...
- `method-business-impact-engineering` ：Value Brief、指标链路、优先级评分、灰度与回滚方案。
- `scenario-feature-design` ：FRD 结构、阶段推进、权限矩阵与风险分析。
- `scenario-prd-visual-optimizer` ：双轨呈现（Mermaid + 表格）与渲染校验。
- `scenario-pm-tech-manager-assistant` ：6D 生命周期、Mermaid 甘特图、沟通矩阵。
- `scenario-ratecard-quote` （如需估算）：生命周期报价结构与 Rate Card 使用。
- `method-stakeholder-communication` （可选）：审批、澄清与记录。
