...
- `method-business-impact-engineering` ：Value Brief、指标链路、灰度/回滚策略、单位经济与实验护栏。
- `scenario-feature-execution` ：解析 FRD、分配任务、执行状态机流程。
- `scenario-quality-gates` ：按目标路径运行格式、静态分析、测试、构建等质量门。
- `method-context-snapshot` （可选）：在长流程中生成会话快照，保持上下文可追溯。
...
4. **逐项执行**：对每个任务应用必要的语言/工具技能（例如 PHP 相关修改加载 `php-core-runtime`、
   `php-tool-phpunit`、`php-tool-phpstan` 等技能），并在完成后立即调用质量门技能。
5. **自愈与升级**：质量门失败时，依照 `scenario-feature-execution` 的自愈策略重试 ≤3 次；无法修复时记录阻塞并请求决策。
6. **汇总结论**：生成执行报告，可用 `--output` 写入文件；如流程较长，结合
   `method-context-snapshot` 输出快照，并回填 Value Brief 状态。

## 产出

- 六块结构化输出：Value Brief 更新、Plan 执行进度、Metrics 触达情况、Ops 风险、
  Code/Config 变更、Risks/下一步决策。
